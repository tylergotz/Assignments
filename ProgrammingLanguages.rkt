...
(define update-env
  (lambda (var val env)
    (cond
      ((null? env) #f)
      (else
       (let ((currentScope (car env)))
        (if (not (eqv? apply-scope (var currentScope) #f))
            (cons (update-scope var val currentScope) (cdr env))
            (cons currentScope (update-env var val (cdr env)))))))))
      ...
      (else
       (cond
         ...
         ((eq? (list-ref (list-ref lce 1) 0) 'block-exp)
          (let ((listOfExpressions (cdr (list-ref lce 2))))
            (eval-block-exp listOfExpressions env)))
